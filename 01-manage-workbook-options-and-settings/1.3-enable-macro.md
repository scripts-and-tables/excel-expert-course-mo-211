...
Office Add-ins (web-based add-ins, distinct from old `.xlam` files) load from a catalog URL—usually a SharePoint app catalog. The **Trusted Add-in Catalogs** page lets administrators register or remove those URLs and control whether catalog-listed add-ins start automatically. For most exam scenarios you only need to know it exists at `Trust Center Settings > Trusted Add-in Catalogs`.
