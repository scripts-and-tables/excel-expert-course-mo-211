# 2.13 Advanced Filter and Database Functions
...
## Part 8: Database Functions (D-Functions)

The **D-functions** apply the same criteria-range pattern as Advanced Filter but return a **single aggregate value** instead of a filtered list. They are the closest thing Excel has to `SELECT SUM(...) FROM ... WHERE ...`.

### Common D-Functions

| Function | Returns | SQL equivalent |
| :--- | :--- | :--- |
| `DSUM(database, field, criteria)` | Sum of a field for rows matching the criteria | `SELECT SUM(field) WHERE …` |
| `DCOUNT(database, field, criteria)` | Count of numeric values in the field | `SELECT COUNT(field) WHERE … AND field IS NUMERIC` |
| `DCOUNTA(database, field, criteria)` | Count of non-blank values in the field | `SELECT COUNT(*) WHERE …` |
| `DAVERAGE(database, field, criteria)` | Average of a field | `SELECT AVG(field) WHERE …` |
| `DMAX(database, field, criteria)` | Max of a field | `SELECT MAX(field) WHERE …` |
| `DMIN(database, field, criteria)` | Min of a field | `SELECT MIN(field) WHERE …` |
| `DGET(database, field, criteria)` | The single value that matches (errors on 0 or 2+ matches) | `SELECT field WHERE …` (one row) |

### Arguments

- **database** — the source list including headers (e.g., `A1:F500`).
- **field** — either the column header in quotes (`"Amount"`) or the numeric column index (`5`).
- **criteria** — the criteria range, identical in structure to the Advanced Filter criteria range.

### Example

Source list `A1:E500` has headers `Region`, `Department`, `Customer`, `Amount`, `Date`. Criteria range built in `H1:I2`:
...
```
=DSUM(A1:E500,"Amount",H1:I2)
```

Returns the sum of `Amount` for every row where Region = North AND Amount > 5000.

> [!NOTE]
> D-functions and Advanced Filter share the same criteria-range syntax — wildcards, comparison operators, computed criteria, and AND/OR rules all behave identically. Build the criteria range once and reuse it for both.
...
- [Microsoft Support — DSUM function](https://support.microsoft.com/en-us/office/dsum-function-53181285-0c4b-4f5a-aaa3-529a322be41b)
- [Microsoft Support — DCOUNT function](https://support.microsoft.com/en-us/office/dcount-function-c1fc7b93-fb0d-4d8d-97db-8d5f076eaeb1)
- [Microsoft Support — DAVERAGE function](https://support.microsoft.com/en-us/office/daverage-function-a6a2d5ac-4b4b-48cd-a1d8-7b37834e5aee)
- [Microsoft Support — DGET function](https://support.microsoft.com/en-us/office/dget-function-9a0a05c2-674f-44b9-be40-b0b7ae46f4d8)
